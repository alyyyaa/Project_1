...
void bigNumber::operator += (bigNumber& n2)
{
	int length;
	if (size > n2.size)
		length = size;
	...
	{
		a[i] += n2.a[i]; // суммируем последние разряды чисел
	a[i + 1] += (a[i] / 10); // если есть разряд для переноса, переносим его в следующий разряд
		a[i] %= 10; // если есть разряд для переноса он отсекается
	}
}
...
void NOD(bigNumber& n1, bigNumber& n2)
{
	while (n1 != n2) {
		if (n1 > n2) {
			n1 -= n2;
